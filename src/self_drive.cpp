...
class SelfDrive : public rclcpp::Node
{
  ...
  int step_;
  // 상수 정의
  const float TARGET_SPEED = 0.22;     // 전진 속도 (m/s)
  const float SAFE_DISTANCE = 0.55;    // 전방 충돌 방지 거리 (m)
  const float ROBOT_WIDTH = 0.28;      // 로봇 지름 (m)
  const float CORRIDOR_WIDTH = 0.4;    // 길 폭 (m)
  
  // P제어 상수 (중앙 정렬을 위한 민감도)
  const float KP_ANGULAR = 2.0; 
public:
  ...
  {
    // 1. 센서 데이터 처리
    float front_dist = get_range_avg(scan, 0, 20);    // 전방 0도 기준 +-10도
    float left_dist = get_range_avg(scan, 40, 15);    // 좌전방 기준 +-15도
    float right_dist = get_range_avg(scan, 320, 15);  // 우전방 기준 +-15도
    // 2. 주행 명령 결정
...
private:
  ...
  geometry_msgs::msg::TwistStamped decide_movement(float front, float left, float right)
  {
    ...
    if (front < SAFE_DISTANCE)
    {
      // 더 넓은 쪽으로 회전
      if (left > right) {
        vel.twist.angular.z = 1.3; // 좌회전 (반시계)
      } else {
        vel.twist.angular.z = -1.3; // 우회전 (시계)
      }
    }
